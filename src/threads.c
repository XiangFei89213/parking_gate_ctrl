...
void* ocr_thread(void* arg){
    ...
    while (1) {
        ...
        // Optional producer jitter (FAULTS=1) — keep OUTSIDE any lock
        if (g_faults) {
            usleep((rand() % 5) * 1000);         // +0–4 ms
        }
...
void* billing_thread(void* arg){
    ...
    while (1) {
        ...
        if (g_bad_lock_order && (rand() % 10 == 0)) {
            // INTENTIONALLY WRONG ORDER (payment -> plate_db)
            // This creates deadlock risk for WFG to catch later.
            pthread_mutex_lock(&m_payment);
            pthread_mutex_lock(&m_plate_db);
            usleep(10 * 1000);                   // simulate quick protected work
            pthread_mutex_unlock(&m_plate_db);
            pthread_mutex_unlock(&m_payment);
        } else {
            // Correct global order: plate_db -> payment
            lock_in_global_order(2);
            usleep(10 * 1000);                   // simulate quick protected work
            unlock_in_global_order(2);
        }
        // ---- End critical section ----

        // Optional non-critical jitter (FAULTS=1) — keep OUTSIDE locks
        if (g_faults) {
            usleep((rand() % 10) * 1000);        // +0–9 ms
        }

        printf("[Billing] OK for %s\n", ev.plate);
        fake_gate_actuate(ev.plate);             // this locks gate_io internally

        // Record end-to-end latency (OCR ts -> here)
        record_latency(&ev);
    }
    return NULL;
}
